...
1. Resolve web infrastructure through `Fl32_Web_` DI tokens; application modules declare
   dependencies through `__deps__` and do not construct DI-managed services directly.
2. Register every handler before `lockHandlers()` or `server.start()`. The lifecycle is
   fixed as `INIT -> PROCESS -> FINALIZE` for the runtime instance.
3. In a `PROCESS` handler, end the response and set `context.completed = true`. Do not use
   removed `context.complete()` or `context.isCompleted()` helpers.
4. Configure runtime transport during bootstrap, including `@teqfw/cfg` loading when its
   `TEQFW_WEB` projection is used; treat frozen runtime configuration as read-only.
5. Read the selected references before editing and validate with the host project's tests.
